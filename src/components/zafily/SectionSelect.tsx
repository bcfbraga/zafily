  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className={`w-full h-11 bg-[#F1F0F7] border ${open ? "border-[#6C63FF] ring-2 ring-[#6C63FF]/20" : "border-black/[0.12] hover:border-black/[0.20]"} text-[#16162B] rounded-xl px-4 text-sm transition-all flex items-center gap-2 cursor-pointer`}
      >
        <Layers className="w-4 h-4 text-[#716C8C] shrink-0" />
        ...
        <div className="absolute z-50 top-[calc(100%+6px)] left-0 right-0 bg-white border border-black/[0.12] rounded-xl shadow-[0_16px_48px_rgba(23,23,60,0.14)] overflow-hidden py-1 max-h-64 overflow-y-auto">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${
              !value ? "bg-[#6C63FF]/10 text-[#4338CA]" : "text-[#4B4768] hover:bg-black/[0.04] hover:text-[#16162B]"
            }`}
          >
            <span className="flex-1 text-left font-medium">Sem seção</span>
            {!value && <Check className="w-4 h-4 text-[#6C63FF]" />}
          </button>
          ...
          {!loading && sections.map(section => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${
                section.id === value ? "bg-[#6C63FF]/10 text-[#4338CA]" : "text-[#4B4768] hover:bg-black/[0.04] hover:text-[#16162B]"
              }`}
            >
              <span className="flex-1 text-left font-medium truncate">{section.name}</span>
              {section.id === value && <Check className="w-4 h-4 text-[#6C63FF]" />}
            </button>
          ))}
            ...
            {creating ? (
              <div className="flex items-center gap-2 px-3 py-1.5">
                <input
                  ...
                  className="flex-1 h-8 bg-[#F1F0F7] border border-black/[0.12] rounded-lg px-2.5 text-xs focus:outline-none focus:border-[#6C63FF]"
                />
                <button type="button" onClick={handleCreate} className="h-8 px-2.5 bg-[#6C63FF] hover:bg-[#5851E0] text-white text-xs font-semibold rounded-lg transition-colors">
                  Criar
                </button>
              </div>
            ) : (
              <button
                ...
                className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-[#6C63FF] hover:bg-[#6C63FF]/5 transition-colors"
              >
